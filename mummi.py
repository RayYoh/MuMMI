...
import tools
import wrappers
from config import define_config, define_config_debug
...
class CMDreamer(tools.Module):
    ...
    def policy(self, obs, state, training):
        ...
        w_touch = obs["state_flag"] / prob_touch["std"]
        embed = w_img * prob_img["mean"]
        embed = embed + w_touch * prob_touch["mean"]
        embed = embed / (w_img + w_touch + 0.01)
    ...
    def _train(self, data, log_images):
        with tf.GradientTape() as model_tape:
            # encoder
            # data['image'].shape=(50, 50, 64, 64, 3)
            embed_img = self._encode_img(data)
            # print(embed_img.shape.as_list())
            ...
            w_touch = data["state_flag"] / prob_touch["std"]
            embed = w_img * prob_img["mean"]
            embed = embed + w_touch * prob_touch["mean"]
            embed = embed / (w_img + w_touch + 0.01)
            ...
            constrast_touch = 10 * self._contrastive(feat_embed, prob_touch["mean"], data["state_flag"])
    ...
    def _build_model(self):
        ...
        self._embed2p_img = models.Embed2prob(50, modality="image")
        self._embed2p_touch = models.Embed2prob(50, modality="state")
        self._dynamics = models.RSSM(self._c.stoch_size, self._c.deter_size, self._c.deter_size)
        self._z2embed = models.Dense(n=1, d_hidden=None, d_out=50)
        ...
        model_modules = [self._encode_img, self._encode_touch,
                         self._embed2p_img, self._embed2p_touch,
...
def preprocess(obs, config):
    ...
    with tf.device('cpu:0'):
        ...
        obs['state'] = tf.cast(obs['state'], dtype) / 200.0
...
def test(config):
    ...
    for miss_r in m_list:
        config.miss_ratio = {"image": miss_r, "state": miss_r, "audio": miss_r}
        test_envs = [wrappers.Async(lambda: make_env(
            config, writer, 'test', datadir, store=False), config.parallel)
                     for _ in range(config.envs)]
